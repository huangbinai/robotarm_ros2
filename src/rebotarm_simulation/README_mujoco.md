...
当前位置。`G` 进入重力补偿，`H` 捕获并保持当前位置，`P` 进入
...
窗口叠加层分区显示系统模式、模式行为、运行/连续运动状态、六轴实际角/目标角/误差/
速度/请求及施加力矩、饱和、Raw torque 看门狗、夹爪和接触状态。终端默认只显示一条
启动提示、用户主动输入命令的结果和错误，不再逐帧刷状态；诊断时可加
`--verbose-status` 恢复实时文本输出。Viewer 默认隐藏右侧原始 Control 面板，
