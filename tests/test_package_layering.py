...
def test_dashboard_delegates_web_gripper_action_lifecycle() -> None:
    ...
    assert "self._web_gripper_client = WebGripperClient(" in source
    assert "self._web_gripper_client.set_position(" in source
    assert "self._web_gripper_client.observe_result(result)" in source
    assert "def _on_gripper_goal_response" not in source
